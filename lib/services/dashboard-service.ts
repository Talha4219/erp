...
function arBucket(agg: { _sum: { totalAmount: unknown; paidAmount: unknown } }) {
  return Math.max(0, Number(agg._sum.totalAmount ?? 0) - Number(agg._sum.paidAmount ?? 0))
}
...
export async function getDashboardData(userId: string, role: string) {
  ...
  const [
    ...
    prisma.expenseCategory.findMany({
      include: { expenses: { where: { deletedAt: null }, select: { amountGbp: true } } },
    }),
    ...
    prisma.workflowInstance.findMany({
      ...
      include: { definition: { select: { name: true, module: true } }, requester: { select: { name: true, email: true } } },
      orderBy: { requestedAt: 'desc' },
      take: 5,
    }),
    prisma.workflowInstance.findMany({
      where: { status: { in: ['PENDING', 'IN_PROGRESS'] } },
      include: { definition: { include: { steps: { orderBy: { stepOrder: 'asc' }, take: 1 } } }, requester: { select: { name: true, email: true } } },
      orderBy: { requestedAt: 'desc' },
    }),
  ...
  const [posToday, posMTD, posRefunds, posCogs] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ net_today: number; net_mtd: number }>>`
      SELECT
        COALESCE(SUM(ABS(sp."amount") / (1 + soi."taxRate")) FILTER (WHERE sp."createdAt" >= ${todayStart}), 0)::float AS net_today,
        COALESCE(SUM(ABS(sp."amount") / (1 + soi."taxRate")), 0)::float AS net_mtd
      FROM "SalesPayment" sp
      JOIN "SalesOrderItemV2" soi ON soi.id = sp."soItemId"
      WHERE sp."method" = 'REFUND' AND sp."createdAt" >= ${monthStart}
    `,
    prisma.$queryRaw<Array<{ cogs_today: number; cogs_mtd: number }>>`
      SELECT COALESCE(SUM(CASE WHEN "referenceType" = 'POS' THEN "totalCost" ELSE -"totalCost" END) FILTER (WHERE "transactionDate" >= ${todayStart}), 0)::float AS cogs_today,
  ...
  const rawRevenue = await prisma.$queryRaw<Array<{ yr: number; mo: number; revenue: number }>>`
    ...
    WHERE status IN ('PAID', 'PARTIALLY_PAID') AND "deletedAt" IS NULL AND "invoiceDate" >= ${twelveMonthsAgo}
    GROUP BY yr, mo
  `
  ...
  const monthlyRevenue = Array.from({ length: 12 }, (_, i) => {
    ...
    return { month: d.toLocaleString('default', { month: 'short' }), year: d.getFullYear(), revenue: revenueByKey[`${d.getFullYear()}-${d.getMonth() + 1}`] ?? 0 }
  })

  const revenueMTDVal = revenueByKey[`${year}-${now.getMonth() + 1}`] ?? 0
  const totalCash = bankAccounts.reduce((s, b) => s + Number(b.currentBalance), 0)

  const lowStockList = lowStockItems.map((item) => ({
    id: item.id, name: item.name, sku: item.sku,
    reorderPoint: Number(item.reorderPoint),
    currentStock: item.warehouseStocks.reduce((s, ws) => s + Number(ws.quantity), 0),
  ...
  const myTasks = allPendingWorkflow
    .filter((w) => {
      ...
      return isAdmin || step.approverId === userId || (step.approverRole && step.approverRole === myRole)
    })
    .slice(0, 8)
    .map((w) => ({
    id: w.id, workflow: w.definition.name, module: w.definition.module,
    requester: w.requester.name ?? w.requester.email, requestedAt: w.requestedAt,
  }))

  const crossModuleActivity = [
    ...recentInvoices.map((inv) => ({ id: inv.id, type: 'invoice', description: `Invoice ${inv.invoiceNumber} for ${inv.customer.name}`, amount: Number(inv.totalAmount), date: inv.invoiceDate, status: inv.status })),
    ...recentPOs.map((po) => ({ id: po.id, type: 'po', description: `PO ${po.poNumber} for ${po.vendor.name}`, amount: Number(po.grandTotal), date: po.createdAt, status: po.status })),
    ...recentPRs.map((pr) => ({ id: pr.id, type: 'pr', description: `Purchase Requisition ${pr.prNumber}`, amount: Number(pr.totalAmount), date: pr.createdAt, status: pr.status })),
    ...recentLeaves.map((l) => ({ id: l.id, type: 'leave', description: `Leave request — ${l.employee.firstName} ${l.employee.lastName}`, amount: 0, date: l.createdAt, status: l.status })),
  ].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 8)

  return {
    kpis: {
      revenueMTD: revenueMTDVal,
      openPOs,
      pendingLeaves,
      lowStockAlerts: lowStockItems.filter((i) => i.warehouseStocks.reduce((s, ws) => s + Number(ws.quantity), 0) <= Number(i.reorderPoint ?? 0)).length,
    ...
    arAging: { current: arBucket(arCurrent), days30: arBucket(ar30), days60: arBucket(ar60), days90plus: arBucket(ar90plus) },
    monthlyRevenue,
    lowStockItems: lowStockList,
    expenseBreakdown: expenseCategories.map((c) => ({ name: c.categoryName, value: c.expenses.reduce((s, e) => s + Number(e.amountGbp), 0) })).filter((c) => c.value > 0),
    recentActivity: crossModuleActivity,
    bankAccounts,
    recentNotifications,
    pendingWorkflow: pendingWorkflow.map((w) => ({ id: w.id, workflow: w.definition.name, module: w.definition.module, requester: w.requester.name ?? w.requester.email, requestedAt: w.requestedAt, status: w.status })),
    topCustomers,
    workflowAlerts,
    myTasks,
  }
...
export async function getDashboardSnapshot() {
  ...
  const [revenueToday, arOutstanding, pendingApprovals, openSO, lowStock] = await Promise.all([
    prisma.customerPayment.aggregate({ where: { paymentDate: { gte: todayStart } }, _sum: { amount: true } }),
    prisma.customerInvoice.aggregate({ where: { status: { notIn: ['PAID', 'CANCELLED'] }, deletedAt: null }, _sum: { totalAmount: true, paidAmount: true } }),
    prisma.workflowInstance.count({ where: { status: 'PENDING' } }),
    prisma.salesOrder.count({ where: { status: { notIn: ['DELIVERED', 'CANCELLED'] }, deletedAt: null } }),
    prisma.warehouseStock.count({ where: { quantity: { lte: 0 } } }).catch(() => 0),
  ])
