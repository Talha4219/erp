...
async function main() {
  ...
  // Create items for each product (if not exist) and map to item IDs
  const itemBySku: Record<string, string> = {}
  for (const p of productData) {
    let existing = await prisma.item.findFirst({ where: { sku: p.sku } })
    if (!existing) {
      const catId = p.category === 'Groceries' ? catParts.id : p.category === 'Toiletries' ? catOffice.id : catElectronics.id
      existing = await prisma.item.create({
        data: {
          sku: p.sku,
          name: p.productName,
          categoryId: catId,
          uom: 'PCS',
          standardCost: 0,
          sellingPrice: p.sellingPriceGbp,
          reorderPoint: p.reorderLevel,
          reorderQty: p.reorderLevel * 2,
        },
      })
    }
    itemBySku[p.sku] = existing.id
  }

  // Fetch vendor IDs for supplierId mapping
  const vendorRecords = await prisma.vendor.findMany({ orderBy: { vendorCode: 'asc' } })
  ...
  if (batchCount === 0) {
    ...
    const batches: { itemId: string; supplierId: string; batchNumber: string; quantityOnHand: number; receivedDate: Date; expiryDate: Date | null }[] = []

    for (const p of productData) {
      const isGrocery = p.category === 'Groceries'
      const isToiletry = p.category === 'Toiletries'
      const supplierVendorId = isGrocery ? vendorIds[0] : isToiletry ? vendorIds[1] : vendorIds[2]

      batches.push(
        {
          itemId: itemBySku[p.sku], supplierId: supplierVendorId,
  ...
  // 10 POS SalesOrderV2 seed entries (only if none exist)
  const soCount = await prisma.salesOrderV2.count({ where: { channel: 'POS' } })
  if (soCount === 0) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Find matching Item by product SKU
      const item = await prisma.item.findFirst({ where: { sku } })
      const itemId = item?.id ?? null

      await prisma.salesOrderV2.create({
        data: {
          orderNumber: `POS-SEED-${i + 1}`,
          channel: 'POS',
          orderType: 'CASH',
  ...
  if (expCount === 0) {
    await prisma.expense.createMany({
      data: [
        { expenseDate: new Date('2026-06-01'), categoryId: expCatRent.id, vendorId: null, description: 'Monthly shop rent — June 2026', amountGbp: 3500.00, vatClaimedGbp: 0, paymentDueDate: new Date('2026-06-15'), status: 'Paid' },
        { expenseDate: new Date('2026-06-05'), categoryId: expCatUtil.id, vendorId: null, description: 'Electricity & Gas — May 2026', amountGbp: 420.00, vatClaimedGbp: 70.00, paymentDueDate: new Date('2026-06-20'), status: 'Paid' },
        { expenseDate: new Date('2026-06-10'), categoryId: expCatWage.id, vendorId: null, description: 'Staff wages week ending 2026-06-07', amountGbp: 2800.00, vatClaimedGbp: 0, paymentDueDate: null, status: 'Paid' },
        { expenseDate: new Date('2026-06-18'), categoryId: expCatMaint.id, vendorId: vendorIds[2], description: 'EPOS system service contract', amountGbp: 350.00, vatClaimedGbp: 58.33, paymentDueDate: new Date('2026-07-18'), status: 'Unpaid' },
        { expenseDate: new Date('2026-06-22'), categoryId: expCatMarket.id, vendorId: null, description: 'Social media advertising — June', amountGbp: 200.00, vatClaimedGbp: 33.33, paymentDueDate: new Date('2026-07-05'), status: 'Unpaid' },
      ],
    })
  }
