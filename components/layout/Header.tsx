...
export function Header() {
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-border/60 bg-background/95 backdrop-blur-sm px-4 md:px-5 gap-3 flex-shrink-0 sticky top-0 z-30">
      {/* Breadcrumb */}
        ...
        {breadcrumbs.map((crumb, i) => (
          <span key={crumb.href} className="flex items-center gap-1 min-w-0">
            ...
            {i === breadcrumbs.length - 1 ? (
              ...
              <Link href={crumb.href} className="text-sm text-muted-foreground/60 hover:text-foreground transition-colors truncate">
                {crumb.label}
              </Link>
            )}
          </span>
      ...
      <div className="relative flex-1 max-w-md" ref={searchRef}>
        <div className="relative group">
          ...
          <input
            ...
            className="h-8 w-full rounded-lg border border-border/60 bg-muted/30 py-0 pl-9 pr-8 text-sm outline-none placeholder:text-muted-foreground/40 focus:bg-background focus:border-border focus:ring-2 focus:ring-ring/20 transition-all"
            value={query}
        ...
        {searchOpen && (
          <div className="absolute top-full left-0 right-0 mt-1.5 z-50 rounded-xl border border-border/60 bg-background shadow-xl shadow-black/10 overflow-hidden">
            {searching ? (
              ...
              <ul className="max-h-72 overflow-y-auto">
                {searchResults.map((r, i) => (
                  <li key={`${r.type}-${r.id}`}>
                    ...
                    >
                      <span className={cn('text-[9px] font-semibold px-1.5 py-0.5 rounded-md uppercase tracking-wide flex-shrink-0 whitespace-nowrap', RESULT_BADGE[r.type] ?? 'bg-muted text-muted-foreground')}>
                        {r.type}
                      </span>
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        ...
        <div className="relative" ref={createRef}>
          ...
          {createOpen && (
            <div className="absolute right-0 top-full mt-1.5 z-50 w-64 rounded-xl border border-border/60 bg-background shadow-xl shadow-black/10 overflow-hidden">
              {QUICK_CREATES.map((group) => (
                <div key={group.group} className="border-b border-border/40 last:border-0">
                  <p className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60 bg-muted/30">
                    {group.group}
                  </p>
        ...
        <div className="relative" ref={notifRef}>
          <button
            className={cn(
              'relative flex h-8 w-8 items-center justify-center rounded-lg border border-border/60 text-muted-foreground hover:text-foreground hover:bg-accent transition-all',
          ...
          {notifOpen && (
            <div className="absolute right-0 top-10 z-50 w-80 rounded-xl border border-border/60 bg-background shadow-xl shadow-black/10 overflow-hidden">
              ...
              <div className="max-h-80 overflow-y-auto divide-y divide-border/30">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    ...
                    >
                      <div className={cn('mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full', TYPE_DOT[n.type] ?? 'bg-gray-400')} />
                      <div className="flex-1 min-w-0">
                        <p className={cn('text-sm truncate', !n.isRead ? 'font-semibold text-foreground' : 'text-foreground/80')}>{n.title}</p>
                      ...
                      {!n.isRead && (
                        <button
                          ...
                          className="flex-shrink-0 mt-1 text-muted-foreground/50 hover:text-blue-600 transition-colors"
                          title="Mark read"
                        >
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className={cn(
              'flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-[11px] font-bold text-white transition-all ring-2 ring-transparent hover:ring-primary/30',
          ...
          {profileOpen && (
            <div className="absolute right-0 top-10 z-50 w-56 rounded-xl border border-border/60 bg-background shadow-xl shadow-black/10 overflow-hidden">
