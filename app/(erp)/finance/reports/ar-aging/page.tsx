...
export default function ARAgingPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<AgingData>('/api/finance/reports/ar-aging', { asOf: applied }).then((r) => r.data),
  })

  return (
    <div className="p-6 space-y-6">
      <PageHeader title="AR Ageing Report" description="Outstanding customer invoices by age" />
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(data?.rows ?? []).length === 0 ? (
                <tr><td colSpan={8} className="text-center py-8 text-muted-foreground">No outstanding invoices</td></tr>
              ) : (
                (data?.rows ?? []).map((row) => (
