...
export function PageClient({ customers }: { customers: Customer[] }) {
  ...
  function updateLine(i: number, field: keyof LineItem, raw: string) {
    setLines((prev) => {
      ...
      next[i] = {
        ...next[i],
        [field]: field === 'description' ? raw : Number(raw),
      }
      return next
    })
  }
  ...
  const lineTotal = (li: LineItem) =>
    li.quantity * li.unitPrice * (1 - li.discount / 100) * (1 + li.taxRate / 100)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!dueDate) return toast.error('Please enter a due date')
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => router.push('/sales/invoices')}>Cancel</Button>
